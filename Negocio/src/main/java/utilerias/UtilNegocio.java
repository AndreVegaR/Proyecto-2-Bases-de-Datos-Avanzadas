...
public class UtilNegocio {
    ...
    /**
     * Este arreglo de Strings define qué campos de los atributos de cualquier clase son opcionales
     * Sirve para ser comparados en el método esCampoOpcional, auxiliar de stringsVacios
     * Los elementos del arreglo son los NOMBRES LITERALES DEL ATRBIBUTO EN JAVA
     * Si coincide, no hace la validación de que si es blank o nulo: puede existir vacío
     */
    private static String[] CAMPOS_OPCIONALES = {"correo"};
    
    //Formato para normalizar una fecha hora en un string cómodo de leer
    public static DateTimeFormatter FORMATO_FECHA_HORA = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm");
    
    /**
     * Compara que el campo del parámetro se halle en el arreglo de campos opcionales
     * Es auxiliar de stringsVacios
     * Si hay una coincidencia, lo deja pasar
     * 
    ...
    /**
     * Método reflexivo que valida si sus campos (por lo menos strings) no sean blank
     * Es genérico en el ámbito de que puede procesar cualquier tipo de objeto
     * La reflexión consiste en que el código se analiza en tiempo de ejecución
     * Es una herramienta poderosa: con decir que puede acceder a atributos privados es suficiente
     * 
    ...
    public static boolean stringsVacios(Object objeto) {
        ...
        //Excepción si algo sale mal
            ...
            throw new NegocioException("Todos los campos especificados de " + objeto.getClass().getSimpleName() + "deben obligatorios");
        }
        return false;
    }
    ...
    /**
     * Une las dos validaciones más importantes: de String y objeto null
     * El combo ganador, el clásico
     * esNulo() y stringsVacios() siguen siendo públicos porque se puede necesitar
     * @param o 
     */
     ...
     * Algo desactualizada 👀 👀 👀 👀, revisar el método stringsVacios()
     * La dejé para no fregarme trabajo ajeno
     * 
     * @param cadenaTexto
     * @param nombreCampo 
     */
